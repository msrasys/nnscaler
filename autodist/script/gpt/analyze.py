...
model_setting_list = ['760M', '1.3B', '2.6B', '6.7B'
                     ] if args.pp else ['350M', '760M', '1.3B', '2.6B', '6.7B']
...
for recompute in recompute_list:
    ...
    for model_setting in model_setting_list:
        for batch_size in batch_size_list:
            table[batch_size] = {}
            fname = './' + args.save_folder + '/gpt3-' + model_setting + '-' + str(
                gpus[model_setting]) + 'gpu-' + str(
                    batch_size) + 'batch_size-' + str(args.is_train)
