...
for ((k=0; k<${#mesh_cols[*]}; k=k+1)); do
    for ((j=0; j<${#bs[*]}; j=j+1)); do
        ...
        SINGLE_DEV_MODE=1 python main.py --GPT_setting=${model_config[k]} --is_train --recompute \
                 ...
                 --memory_granularity=$memory_granularity --save_folder=$save_folder --connect_type=$connect_type --mesh_row=${mesh_rows[k]} --mesh_col=${mesh_cols[k]} --compile \
                 --topk=$topk --fine_grained_GPT  --ignore_small_tensor_threshold=1048576 \
                 --verbose  --adaptive_recom

        for (( i=0; i < $topk; ++i))
        do
            torchrun --nnodes=${mesh_rows[k]} --nproc_per_node=${mesh_cols[k]} main.py --GPT_setting=${model_config[k]} --is_train --recompute \
                     --micro_batch_size=${bs[j]} --memory_constraint=$memory_constraint  \
                     --memory_granularity=$memory_granularity --save_folder=$save_folder --connect_type=$connect_type \
                     --mesh_row=${mesh_rows[k]} --mesh_col=${mesh_cols[k]} --plan_idx=$i --fine_grained_GPT --adaptive_recom
            if [ $? -eq 0 ]
            then
                echo "success at $i trial"
                break
            else
                echo "fail at $i trial"
            fi
        done
