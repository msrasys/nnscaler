...
@mock_reducer_env(0, 2)
def test_manual_grad_ready_uses_async_vpp_expected_contribution_count(monkeypatch):
    param = torch.nn.Parameter(torch.zeros(2))
...
@mock_reducer_env(0, 2)
def test_manual_grad_combines_with_regular_autograd_path_before_async_ready(monkeypatch):
    param = torch.nn.Parameter(torch.zeros(2))
    reducer = Reducer([0, 1], async_op=True)
    reducer.add_param(param)
    reducer.build_buckets()
