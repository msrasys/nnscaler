...
def test_async_send_callback_runs_when_drained(monkeypatch):
    class Work:
        def __init__(self):
            self.waited = False
    ...
    monkeypatch.setattr(executor_module, '_instance', _AsyncCommHandler())
    try:
        work = Work()
        called = {'value': False}
        ...
        AsyncCommHandler().drain_sends_completed()
