...
from nnscaler.runtime.adapter.reducer import Bucket
...
def test_schedule_node_uses_chronotrigger_ranges() -> None:
    tree = ast.parse(textwrap.dedent(inspect.getsource(ScheduleNode)))
    calls = {
        ast.unparse(node.func)
        for node in ast.walk(tree)
        if isinstance(node, ast.Call)
    }
    assert "ct.range_from" in calls
    assert "torch.cuda.nvtx.range_push" not in calls
    assert "torch.cuda.nvtx.range_pop" not in calls
    source = inspect.getsource(ScheduleNode)
    assert "self._stream_ctx(ct.Kind.FWD)" in source
    assert "self._stream_ctx(ct.Kind.BWD)" in source
...
def test_bucket_records_split_backward_contribution_counts() -> None:
    ...
    bucket._async = True
    bucket._params = [parameter, other]
    bucket._async_expected_param_cnt = {}
    bucket._async_seen_param_cnt = {}
    bucket._async_expected_total = 0
    ...
    assert bucket._async_expected_param_cnt == {parameter: 2, other: 0}
    assert bucket._async_expected_total == 2
    assert bucket._async_record_param_counts is None


def test_manual_wait_grads_leaves_framework_sync_gate_enabled() -> None:
    class DummyBucket:
        _async = True
    ...
    module = SimpleNamespace(
        _reducers=[SimpleNamespace(_buckets=[DummyBucket()])],
        _sync_grad_required=True,
    )

    manual_wait_grads(module)

    assert module._sync_grad_required is True
