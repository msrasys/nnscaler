...
def get_bash_arguments(num_gpus_per_node, **kwargs):
    ...
    script_path = os.path.join(project_root, "tests", "customized_ops", "ring_attn", "run_correctness.py")
...
def test_ring_attn_varlen_correctness(dtype, config_name):
    """Test ring attention variable length correctness with different configurations"""
...
def test_ring_attn_varlen_multi_gpu(num_gpus, config_name):
...
@pytest.mark.parametrize("dtype", ["bf16", "fp16"])
def test_ring_attn_varlen_all_configs(dtype):
...
if __name__ == "__main__":
    ...
    test_ring_attn_varlen_correctness("bf16", "small")
