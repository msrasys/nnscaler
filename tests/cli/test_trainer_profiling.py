...
def rankzero_only_stack():
    # rank zero will export stacks, other ranks will not export stacks
    # NOTE: when train args is loaded,
    # torch.distributed might not be initialized yet, so we cannot check rank at that time.
...
def trainer_profiling_worker(save_dir):
    ...
    trainer = Trainer([
        ...
        '--debug.profile.with_stack.__type', 'tests.cli.test_trainer_profiling.rankzero_only_stack',
    ...
    if trainer.rank == 0:
        assert trainer.train_args.debug.profile.with_stack is True
    else:
        assert trainer.train_args.debug.profile.with_stack is False
    ...
    # with_stack is True for rank zero, stack files should not be generated for other ranks
    stack_files = list(trace_dir.glob('stack_rank*_step*.txt'))
    assert stack_files and all(f.stem.startswith('stack_rank0') for f in stack_files)

    if trainer.rank == 0:
        Trainer.merge_checkpoint(list((ckpt_savedir / 'last').glob('*.ckpt')), save_dir / f'merged.pt')
