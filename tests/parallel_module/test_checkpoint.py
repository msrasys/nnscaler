...
def _train(model: torch.nn.Module, num_replicas, rank, start, end, ckpt_dir):
    ...
    if ckpt_start_file.exists():
        ...
        assert 'fc_relu1.CUBE_EXTRA_STATE' in model_state_dict
        assert 'fc_relu2.CUBE_EXTRA_STATE' in model_state_dict
    ...
    assert 'fc_relu1.CUBE_EXTRA_STATE' in model_state_dict
    assert 'fc_relu2.CUBE_EXTRA_STATE' in model_state_dict
    extra_state1 = ExtraState(**model_state_dict['fc_relu1.CUBE_EXTRA_STATE'])
...
def _gpu_worker(pas, plan_ngpus, runtime_ngpus, per_resume_update_count, resume_count):
    init_distributed()
    compiled_results = []
...
def test_checkpoint():
    cube_results = launch_torchrun(4, _gpu_worker, PASRandomSPMD, 2, 4, 32, 1)
    rcube_results = launch_torchrun(4, _gpu_worker, PASRandomSPMD, 2, 4, 16, 2)
