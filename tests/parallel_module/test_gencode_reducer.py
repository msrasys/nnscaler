...
def test_codegen_norm_linear_split(tmp_path):
    ...
    parallelize(
        ...
        {'x': torch.randn(2, 4)},
        normal_linear_split_policy,
        ComputeConfig(2, 4, reducer_replicated_weights=False),
    ...
    parallelize(
        ...
        {'x': torch.randn(2, 4)},
        normal_linear_split_policy,
        ComputeConfig(2, 4, reducer_replicated_weights=True),
...
def no_grad_reduce_matmul(w, x):
    return w @ x


def no_grad_reduce_matmul_split_policy(graph, cfg):
    from nnscaler.policies import get_pas_ops, OpPlan, OpPartition

    for node in get_pas_ops(graph):
        if node.fn == no_grad_reduce_matmul:
            yield OpPlan(node, partition=OpPartition(input=1, dim=1))


class NoGradReduceMatmulModule(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.weight = torch.nn.Parameter(torch.randn(4, 4))

    def forward(self, x):
        return no_grad_reduce_matmul(self.weight, x)


@replace_all_device_with('cpu')
def test_codegen_no_grad_reduce_matmul_split(tmp_path):
    m = NoGradReduceMatmulModule()
    m.train()
    parallelize(
        m,
        {'x': torch.randn(4, 4)},
        no_grad_reduce_matmul_split_policy,
        ComputeConfig(2, 4),
        gen_savedir=tmp_path,
        load_module=False,
        reuse='override',
    ...
    # weight is partitioned, so the reducer should have rank=[0, 2] and nreplicas=1
    assert _gencode_contains(tmp_path, NoGradReduceMatmulModule, 0,
        r"nnscaler.runtime.adapter.Reducer\(ranks=\[0, 2\].*nreplicas=1\)",
        instance_name = 'noreplicated_weights',
    )
    # generated reducer:
    ...
    # generated reducer:
    # self.wreducer32 = nnscaler.runtime.adapter.Reducer(ranks=[0, 1, 2, 3], reduce_op='sum', async_op=async_op, zero=0, max_bucket_size_bytes=max_bucket_size_bytes, zero_use_reduce_scatter=zero_use_reduce_scatter, zero_param_level_sharding=zero_param_level_sharding,zero_ngroups=1,nreplicas=2)
