...
class NarrowSegmentModel(torch.nn.Module):
    """Model to test segment boundary narrowing.

    Two pipeline stages: stage 0 produces column-partitioned output,
    stage 1 consumes it with the same column partition.
    Narrowing should allow direct P2P without AllGather at the boundary.
    """
    def __init__(self):
        super().__init__()
        self.w1 = torch.nn.Parameter(torch.randn(4, 4))
    ...
    def forward(self, x):
        # x: (4, 4)
        y = torch.matmul(x, self.w1)   # stage 0, partition w1 on dim=1 → output (4, 2) per device
        z = torch.nn.functional.relu(y) # stage 1, partition y on dim=1 → needs (4, 2) per device
        return z.sum()


def _narrow_segment_policy(graph, cfg):
    """Pipeline policy: stage 0 partitions matmul output columns, stage 1 partitions relu input columns.

    Stage 0: matmul(x, w1) with OpPartition(input=1, dim=1) → w1 split on columns
    Stage 1: relu(y) with OpPartition(input=0, dim=1) → y split on same columns
    Matching partition means no allgather needed at the stage boundary.
    """
    from nnscaler.policies import OpPlan, OpPartition, get_pas_ops
    ...
    for node in get_pas_ops(graph):
        if matmul_found:
            if node.fn == torch.nn.functional.relu:
                yield OpPlan(node, stage_id=1, partition=OpPartition(input=0, dim=1))
            else:
                yield OpPlan(node, stage_id=1)
        else:
            if node.fn == torch.matmul:
...
@replace_all_device_with('cpu')
def test_narrow_segment_boundary(tmp_path):
    """Test that narrowing analysis functions correctly identify partition opportunities.

    Stage 0 produces y with columns partitioned (dim=1: each device has half columns).
    Stage 1 consumes y also partitioned on dim=1 (same split).

    The analysis functions should correctly detect:
    - Producer narrowing: stage 0's internal per-device partitions can be exposed
    - Consumer narrowing: stage 1's internal per-device consumption (through identity)
      can be identified
    """
    import glob
    from nnscaler.graph.graph import IRGraph
    from nnscaler.graph.segment import IRSegment
    from nnscaler.graph.gener.gen import _try_narrow_segment_ptensors, _try_narrow_segment_ctensors
    from nnscaler import policies

    m = NarrowSegmentModel()
    m.train()
    cc = ComputeConfig(4, 4, use_end2end=True, pas_config={
        'pipeline_nmicros': 2, 'pipeline_size': 2,
    })
    parallelize(m, {'x': torch.randn(4, 4)}, _narrow_segment_policy, cc,
                gen_savedir=tmp_path, load_module=False, reuse='override')

    # Load pre-policy graph and re-apply policy to get the post-partition state
    # (before adapter generation, which is where narrowing analysis applies)
    ckp = glob.glob(str(tmp_path / '**' / 'graph.ckp'), recursive=True)[0]
    graph = IRGraph.load(ckp)
    graph = policies.fn(graph, cc, policy=_narrow_segment_policy)

    # Find forward segments
    fw_segments = [seg for seg in graph.nodes() if isinstance(seg, IRSegment) and seg.isfw()]
    assert len(fw_segments) == 2

    # Find the inter-stage ftensor (produced by one segment, consumed by another)
    inter_stage_ftensor = None
    stage0_seg = stage1_seg = None
    for ftensor in graph.full_tensors():
        prod_segs = [p for p in graph.producers(ftensor) if isinstance(p, IRSegment) and p.isfw()]
        cons_segs = [c for c in graph.consumers(ftensor) if isinstance(c, IRSegment) and c.isfw()]
        if prod_segs and cons_segs and prod_segs[0] != cons_segs[0]:
            inter_stage_ftensor = ftensor
            stage0_seg = prod_segs[0]
            stage1_seg = cons_segs[0]
            break

    assert inter_stage_ftensor is not None, "Could not find inter-stage tensor"
    assert inter_stage_ftensor.shape == (4, 4)

    # Test producer narrowing: stage 0 should have per-device partitions
    producer_result = _try_narrow_segment_ptensors(stage0_seg, inter_stage_ftensor)
    assert producer_result is not None, "Producer narrowing should identify partitions in stage 0"
    assert len(producer_result) == 2
    # Each partition should be on a different device within stage 0
    devices = sorted(pt.device[0] for pt in producer_result)
    assert devices == [0, 1]
    # Partitions should cover dim=1 (half each)
    indmaps = sorted(pt.indmap for pt in producer_result)
    assert indmaps == [((0, 4), (0, 2)), ((0, 4), (2, 4))]

    # Test consumer narrowing: stage 1 should identify partitions through identity trace
    consumer_result = _try_narrow_segment_ctensors(stage1_seg, inter_stage_ftensor)
    assert consumer_result is not None, "Consumer narrowing should identify partitions in stage 1"
    assert len(consumer_result) == 2
    # Each partition should be on a different device within stage 1
    devices = sorted(ct.device[0] for ct in consumer_result)
    assert devices == [2, 3]
    # Partitions should match the producer's pattern (same dim=1 split)
    indmaps = sorted(ct.indmap for ct in consumer_result)
    assert indmaps == [((0, 4), (0, 2)), ((0, 4), (2, 4))]


@replace_all_device_with('cpu')
def test_narrow_segment_codegen(tmp_path):
    """Test that producer narrowing eliminates redundant allgather+chunk in generated code.

    After narrowing:
    - Stage 0 (ranks 0,1) should return a single partition (shape 4x2) per device
    - The forward adapter should P2P send the partition directly (no allgather)
    - The backward adapter should P2P receive gradient directly (no allgather)
    - Stage 1 (ranks 2,3) should receive partition directly via adapter
    - Backward should use single output_tensor and single output_grad
    """
    from tests.parallel_module.test_gencode import print_gencode
    ...
    # Capture generated code for rank 0 (producer)
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
        print_gencode(tmp_path, NarrowSegmentModel, 0)
    rank0_code = buf.getvalue()

    # Segment should return a single matmul value (not a tuple of two partitions)
    assert re.search(r'return matmul_\d+\s*$', rank0_code, re.MULTILINE)
    # Backward should use single output tensor and single gradient (each in a 1-tuple)
    bwd_match = re.search(r"backward\('segment\d+', \(\), \((\w+), \), \((\w+), \)\)", rank0_code)
    assert bwd_match, "Backward should use single output_tensor and single gradient"
    # The output tensor should be a matmul result
    assert bwd_match.group(1).startswith('matmul_')
    # The gradient should be a gmatmul result
    assert bwd_match.group(2).startswith('gmatmul_')

    # Capture generated code for rank 2 (consumer)
    buf = io.StringIO()
    with contextlib.redirect_stdout(buf):
        print_gencode(tmp_path, NarrowSegmentModel, 2)
    rank2_code = buf.getvalue()

    # Consumer's segment should take partition as input
    assert re.search(r'def segment\d+\(self, matmul_\d+\)', rank2_code)
