...
def print_gencode(cubesave_dir, module_class, index=0, instance_name=None):
...
# class NormLinearModule2(torch.nn.Module):
#     def __init__(self):
#         super().__init__()
#         self.weight = torch.nn.Parameter(torch.randn(8, 4))

#     def forward(self, x):
#         weight = self.weight + 3
#         return F.linear(x, weight)

# class NormLinearModule3(torch.nn.Module):
#     def __init__(self):
#         super().__init__()
#         self.weight = torch.nn.Parameter(torch.randn(8, 4))

#     def forward(self, x, y):
#         return F.linear(x, self.weight) + F.linear(y, self.weight)


# def norm_split_policy(graph, cfg):
#     from nnscaler.policies import get_pas_ops, OpPlan, OpPartition
...
# @replace_all_device_with('cpu')
# def test_codegen_norm2(tmp_path):
#     m = NormLinearModule3()
#     m.train()
#     parallelize(
#         m,
...
#         reuse='override',
#     )
#     print_gencode(tmp_path, NormLinearModule3, 0)
#     print_gencode(tmp_path, NormLinearModule3, 1)
#     for rank in range(2):
#         # Should have wreducer for weight gradient accumulation
#         assert _gencode_contains(tmp_path, NormLinearModule3, rank, r'wreducer')



# @replace_all_device_with('cpu')
# @pytest.mark.parametrize('module_cls', [NormLinearModule1, NormLinearModule2])
# def test_codegen_norm(tmp_path, module_cls):
#     m = module_cls()
#     m.train()
#     parallelize(
#         m,
...
#         reuse='override',
#     )
#     print_gencode(tmp_path, module_cls, 0)
#     print_gencode(tmp_path, module_cls, 1)
#     for rank in range(2):
#         # Should NOT generate identity_allreduce (backward allreduce deferred to weight reducer)
#         assert not _gencode_contains(tmp_path, module_cls, rank, r'identity_allreduce')
#         # Should have wreducer for weight gradient accumulation
#         assert _gencode_contains(tmp_path, module_cls, rank, r'wreducer')
