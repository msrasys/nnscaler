...
`__type` and `__value_type` are also supported for dataclass members. For example, you can specify the `instance_name` argument in `TrainerArgs` as

``` python
instance_name: {
    '__type': 'fqn.to.some.function.that.returns.str',
    'arg0': 'arg0_value',
    ...
}
```
...
Use `$$` to produce a literal dollar sign and escape the variable
syntax. For example, `$$(foo.bar)` produces the literal string
`$(foo.bar)` instead of resolving it as a variable reference.

Note: `$$` alone will still be resolved to `$$`. Only `$$` followed by `(` or `{` will be treated as an escaped variable reference. For example, `$$foo` will be resolved to `$$foo`, not `$foo`.
...
Use `$$!` to produce a literal `$!` and escape the resolved-reference
syntax. For example, `$$!(self)` produces the literal string `$!(self)`
instead of being resolved.

Note:
1. This escaping only happens in component arguments, not in other places. For example, `$!(self)` in `instance_name` will be `$!(self)`, no resolved reference or escaping will happen there.
2. `$$!` alone will still be resolved to `$$!`. Only `$$!` followed by `(` or `{` will be treated as an escaped resolved reference. For example, `$$!foo` will be resolved to `$$!foo`, not `$!foo`.
