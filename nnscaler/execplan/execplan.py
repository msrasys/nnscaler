        ...
        def align_with_dispatched(cell_objs, reuse_objs, dispatched_objs):
            ...
            for dispatched_obj in dispatched_objs:
                ...
                for idx, (cell_obj, _) in enumerate(pairs):
                    if idx in used:
                        continue
                    if isinstance(cell_obj, IRSubTensor) and isinstance(dispatched_obj, IRSubTensor):
                        # dispatch preserves parent.tid; shapes may differ due to narrowing
                        if cell_obj.parent.tid == dispatched_obj.parent.tid:
                            match_idx = idx
                            break
                    elif cell_obj is dispatched_obj or cell_obj == dispatched_obj:
                        match_idx = idx
                        break
