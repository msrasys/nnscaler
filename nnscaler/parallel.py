...
class ComputeConfig:
    ...
    # whether to issue cross-stage point-to-point receives (irecv) asynchronously
    # and defer their wait until the first consumer needs the tensor. Combined
    # with operator rescheduling (which hoists the receive earlier), this overlaps
    # the receive with intervening compute. Independent of `use_async_comm`.
    # This option only works for pipeline.
    use_async_recv: bool = False
...
def _compile_flags(compute_config: ComputeConfig):
    return _flags(
        ...
        async_recv=compute_config.use_async_recv,
    )
