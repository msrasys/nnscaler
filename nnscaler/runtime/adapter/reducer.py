...
class Bucket:
    def __init__(self, reducer: 'Reducer', params: List[torch.nn.Parameter],
        ...
        # Manual contributions complete through the normal AccumulateGrad hook.
        ...
        def post_grad_hook(param: torch.nn.Parameter, *unused): # pragma: no cover
            ...
            if param.grad is None:
                if not manually_accumulated:
                    raise RuntimeError(
                        "Reducer received no gradient. Call mark_reducer_grad_ready(param) "
                        "after manually accumulating a gradient before returning None."
                    )
            elif self._z3:
