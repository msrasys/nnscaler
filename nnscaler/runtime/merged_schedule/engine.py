...
class ScheduleNode:
    ...
    def __init__(
        ...
        with ct.scope(f"merged_schedule.{self.name}") as trace_context:
    ...
    def _forward(self, *inputs):
        with self._stream_ctx(ct.Kind.FWD):
            self.inputs = []
            for inp in inputs:
                if inp is None:
    ...
    def _backward(self, *output_grad, retain_graph=False):
        with self._stream_ctx(ct.Kind.BWD):
            if self.checkpoint:
                if self.pass_node:
            ...
            try:
                with torch.cuda.stream(self.stream):
                    with ct.range_from(
                        self.trace_context,
                        kind=kind,
                        entity=f"merged_schedule.{self.name}",
                        process_scope=False,
                    ):
                        yield
            finally:
