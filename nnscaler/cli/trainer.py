        ...
        *,
        ...
        for i in range(1, len(state_dicts)):
            ...
            if state_dicts[i]['train_args']['model'] != state_dicts[0]['train_args']['model']:
                logger.warning(f"model config in {checkpoint_files[i]} is different from {checkpoint_files[0]}")
            if state_dicts[i].get('lr_scheduler', None) != state_dicts[0].get('lr_scheduler', None):
                logger.warning(f"lr_scheduler state in {checkpoint_files[i]} is different from {checkpoint_files[0]}")
