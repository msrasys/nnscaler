...
class IRAdapterGener:
    ...
    def _get_gen_reducer_info(
        ...
        Known limitations of arbitrary custom policies (not generated by policies.fn):
            The cases below can compile with an incorrect reducer replica count;
            they are not currently rejected here. They concern full local gradients,
            not complementary value partitions that require a plain SUM.

            policies.fn uses a uniform tp_size = plan_ngpus // pipeline_size and
            assigns stage s to devices [(s % pipeline_size) * tp_size,
            ((s % pipeline_size) + 1) * tp_size). Thus stage device groups are
            identical or disjoint and equally sized. Private-parameter consumers
            also use the whole TP group; spatial partitioning preserves their
            output parent identities across ranks. Normal OpPlan-based policies
            therefore do not generate any of the following three patterns.

            Shared compile-only example for the three cases::

                @nnscaler.register_op('a b, b : / -> a b')
                def full_grad_mul(x, weight):
                    return x * weight

                class Model(torch.nn.Module):
                    def __init__(self, private_last=False):
                        super().__init__()
                        self.weight = torch.nn.Parameter(torch.ones(128))
                        self.private = (
                            torch.nn.Parameter(torch.ones(128)) if private_last else None
                        )

                    def forward(self, x):
                        x = full_grad_mul(x, self.weight)  # s0
                        x = full_grad_mul(x, self.weight)  # s1
                        w = self.weight if self.private is None else self.private
                        return full_grad_mul(x, w).sum()   # s2

                def policy(graph, cfg):
                    nodes = graph.select(ntype=IRFwOperation)
                    graph.staging(nodes[:3])
                    stages = [
                        s for s in graph.select(ntype=IRSegment, flatten=False) if s.isfw()
                    ]
                    for stage, ranks in zip(stages, stage_devices):
                        for node in stage.nodes():
                            if len(ranks) == 1:
                                graph.assign(node, ranks[0])
                            else:
                                if node.name == 'identity':
                                    parts = graph.replicate(node, len(ranks))
                                else:
                                    parts = graph.partition(
                                        node, node.algorithm('dim'), idx=0, dim=0,
                                        num=len(ranks),
                                    )
                                for rank, part in zip(ranks, parts):
                                    graph.assign(part, rank)
                    for node in graph.select(ntype=IRDataOperation):
                        for rank, part in enumerate(graph.replicate(node, cfg.plan_ngpus)):
                            graph.assign(part, rank)
                    # Serialize stages to avoid conflicts on overlapping devices.
                    schedule = SchedulePlan(graph, 1)
                    for step, stage in enumerate(stages + [s.mirror for s in reversed(stages)]):
                        schedule.add_segment(stage, 0, step)
                    schedule.finish()
                    graph.bind_schedule(schedule)
                    return graph

            The '/' annotation declares a full weight gradient on each TP rank.
            This is a metadata/codegen example, NOT a numerical training recipe:
            multiplication alone does not implement the communication needed to
            make that declaration true for sharded inputs. Below, g0/g1/g2 denote
            complete contributions from s0/s1/s2 under the declared semantics.

            1. Partially overlapping groups with equal contribution multiplicity::

                    stage_devices = ((0, 1), (0, 1), (1, 2))
                    private_last = False

                Rank 0 holds g0+g1, rank 1 holds g0+g1+g2, rank 2 holds g2.
                Their consumer sets differ, so each inferred device group has size 1.
                The generated SUM uses nreplicas=1 and produces 2*(g0+g1+g2);
                it would need nreplicas=2, or this placement should be rejected.

            2. Unequal contribution multiplicities on overlapping groups::

                    stage_devices = ((0, 1), (0, 1), (0,))
                    private_last = False

                Rank 0 holds g0+g1+g2; rank 1 holds g0+g1. Both inferred groups
                have size 1, and SUM produces 2*g0+2*g1+g2. No single replica
                divisor can correct all contributions; this placement needs
                separate gradient handling or should be rejected.

            3. An unrelated private parameter splits a shared weight's device group::

                    stage_devices = ((0, 1), (0, 1), (0,))
                    private_last = True

                The shared weight has g0+g1 on both ranks, but the private
                parameter's consumer exists only on rank 0. Device groups are
                inferred from consumers across ALL weights, so the private
                consumer splits (0, 1) into singleton groups. The shared weight
                gets nreplicas=1 instead of optional averaging with nreplicas=2
                (or no reducer when reduce_replicated_params=False).
                The private parameter itself needs no reducer. The problem is
                cross-weight device-group inference, not reducer bucket packing.

            After selecting one case, compile it with::

                ngpus = max(rank for ranks in stage_devices for rank in ranks) + 1
                with tempfile.TemporaryDirectory() as output_directory:
                    parallelize(
                        Model(private_last), {'x': torch.ones(128, 128)}, policy,
                        ComputeConfig(ngpus, ngpus, use_end2end=True),
                        gen_savedir=output_directory, load_module=False, reuse='override',
                    )
        """
