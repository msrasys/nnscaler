...
def fn(
    ...
    if ngpus % pp_size != 0:
        raise ValueError(f'invalid pp_size {pp_size} for ngpus {ngpus}')
    op_partition_ngpus = ngpus // pp_size
    ...
    for ftensor, stage_info in tensor_splits.items():
        ...
        if len(splits) > 1 or (
            ...
            # The original parameter and multiref live in the first stage. The
            # other stages consume the new multiref outputs instead.
            stage_info[min_stage_id] = {'rn'}
            for other_stage_id in list(stage_info):
                if other_stage_id != min_stage_id:
                    stage_info.pop(other_stage_id)

            assert len(multiref_node.outputs()) == len(consumers), \
                "Internal Error: multiref outputs should match the number of consumers"
            new_tensor_splits.update(
                _get_new_node_outputs_splits(
                    multiref_node, graph, op_plans, op_partition_maps
                )
            )
        ...
        for stage_id, stage in enumerate(pp_segs):
            for node in stage.select(ntype=IRFwOperation):
                ...
                if len(input_tensors) == 1 and len(output_tensors) == 1:
                    input_ftensor = input_tensors[0].parent
                    output_ftensor = output_tensors[0].parent
                    # Staging identities are replicated. Record both the
                    # identity input and its newly-created output so subsequent
                    # activation multiref decisions see the real consumers.
                    tensor_splits.setdefault(input_ftensor, {})[stage_id] = {'rn'}
                    op_partition_maps[node] = {}
                    tensor_splits.update(
                        _get_new_node_outputs_splits(
                            node, stage, op_plans, op_partition_maps
                        )
                    )
    ...
    for stage_id, seg in enumerate(pp_segs):
        for sub_tensor in IR.get_objects(seg.outputs()):
            ...
            if seg.consumers(sub_tensor.parent):
                ...
                # The identity itself is replicated. Let the split comparison
                # below decide whether the original activation needs multiref;
                # adding one unconditionally breaks no-grad-reduce outputs.
                tensor_splits.setdefault(old_ftensor, {}).setdefault(
                    stage_id, set()
                ).add('rn')
    ...
    for op_plan in op_plans.values():
        ...
        devs = curr_devices[
            idx * op_partition_ngpus:(idx + 1) * op_partition_ngpus
        ]
        if op_plan.partition is not None:
