...
template <typename T> void read_binary(T *ptr, std::ifstream &stream) {
  stream.read(reinterpret_cast<char *>(ptr), sizeof(*ptr));
}

void build_graph(std::ifstream &input) {
  queries.clear();
  id2node.clear();

  int n, q_size;
  read_binary<int>(&mode, input);
  read_binary<int>(&n, input);
  read_binary<int>(&mem_bound, input);
  read_binary<int>(&mem_div, input);
  read_binary<int>(&topk, input);
  read_binary<int>(&q_size, input);

  if (verbose) {
    printf("is big endian: %d\n", is_big_endian());
  }
  printf("node num: %d, mem_bound: %d, mem_div: %d, topk: %d, query num: %d\n",
         n, mem_bound, mem_div, topk, q_size);
  queries.resize(q_size);
  for (int i = 0; i < q_size; ++i) {
    int start, end;
    read_binary<int>(&start, input);
    read_binary<int>(&end, input);
    queries[i] = std::make_pair(start, end);
  }
  for (int i = 0; i < n; ++i) {
    Node *node = new Node();
    read_binary<int>(&node->id, input);
    id2node[node->id] = node;
    read_binary<int>(&node->father_id, input);

    int cut_id;
    read_binary<int>(&node->cut_len, input);
    node->cut_nodes.resize(node->cut_len);
    for (int j = 0; j < node->cut_len; ++j) {
      read_binary<int>(&cut_id, input);
      node->cut_nodes[j] = id2node[cut_id];
    }

    read_binary<int>(&node->p_num, input);
    node->p_father.resize(node->p_num);
    node->p_time.resize(node->p_num);
    node->p_comp_mem.resize(node->p_num);
    node->p_buf_mem.resize(node->p_num);
    node->p_act_mem.resize(node->p_num);
    node->p_opt_mem.resize(node->p_num);
    for (int j = 0; j < node->p_num; ++j) {
      read_binary<double>(node->p_time.data() + j, input);
      read_binary<int>(node->p_comp_mem.data() + j, input);
      read_binary<int>(node->p_buf_mem.data() + j, input);
      read_binary<int>(node->p_act_mem.data() + j, input);
      read_binary<int>(node->p_opt_mem.data() + j, input);
      read_binary<int>(node->p_father.data() + j, input);
    }

    read_binary<int>(&node->producer_num, input);
    node->producers.clear();
    node->comm_costs.clear();
    node->comm_costs.resize(node->producer_num);
    for (int j = 0; j < node->producer_num; ++j) {
      int producer_id;
      read_binary<int>(&producer_id, input);
      Node *producer = id2node[producer_id];
      node->producers.push_back(producer);
      node->comm_costs[j].resize(node->p_num * producer->p_num);
      for (int k = 0; k < node->p_num * producer->p_num; ++k) {
        read_binary<double>(node->comm_costs[j].data() + k, input);
      }
    }
    node->dp_num = 1;
...
void buildInEdges(DPNode *dp_node) {
  ...
  for (int idx = 0; idx < producer_comb_num; ++idx) {
    ...
    for (int j = 0; j < node->producer_num; ++j) {
      ...
      // then the partition of the node should follow the producer's partition,
      // except the producer is the father node.
    ...
    for (int j = 0; j < node->producer_num; ++j) {
      ...
      if (iter != info.end()) {
        ...
        if (producer->father_id != node->father_id) {
          ...
          follow_candidates.push_back(std::make_pair(producer_id, producer_p));
        }
      }
      cost +=
...
void update(DPNode *dp_node, int start_level) {
  ...
  // use a priority queue to maintain the best state, similar to the merge sort
  ...
  for (int i = 0; i < dp_node->in_edges.size(); ++i) {
    DPNode *pred = dp_node->in_edges[i].first;
    int mem = cur_p_comp_mem + std::get<0>(pred->state[0]);
    double cost =
        cur_p_time + dp_node->in_edges[i].second + std::get<1>(pred->state[0]);
  ...
  int cur_opt_mem;
  while (!pq.empty()) {
    std::tie(cur_mem, cur_cost, pred_idx, cur_act_mem, cur_opt_mem) = pq.top();
    cur_mem = -cur_mem;
    cur_cost = -cur_cost;
    ...
    if (dp_node->state.empty()) {
      dp_node->state.push_back(
          std::make_tuple(cur_mem, cur_cost, dp_node->in_edges[pred_idx].first,
                          cur_act_mem, cur_opt_mem));
    } else {
      int pre_mem = std::get<0>(dp_node->state[dp_node->state.size() - 1]);
      double pre_cost = std::get<1>(dp_node->state[dp_node->state.size() - 1]);
...
std::tuple<double, double, int, std::vector<std::pair<int, int>>>
process_state(DPNode *dp_node, int idx) {
  ...
  if (ret_mem > mem_bound) {
    return std::make_tuple(-1, -1, -1, std::vector<std::pair<int, int>>());
  }
  if (verbose) {
    std::cout << "best time: " << best_time
              << ", best mem: " << best_mem / 1024 / 1024 * mem_div << "MB, "
              << "activation mem: " << act_mem / 1024 / 1024 * mem_div << "MB, "
              << "optimizer state mem: " << opt_mem / 1024 / 1024 * mem_div
              << "MB" << std::endl;
  }
  return std::make_tuple(best_time, inner_time, static_cast<int>(ret_mem),
                         path);
}

template <typename T> void write_binary(T val, std::ofstream &stream) {
  stream.write(reinterpret_cast<char *>(&val), sizeof val);
}

void post_process(int start_level, int end_level, int topk,
                  std::ofstream &output) {
  std::vector<std::tuple<double, double, int, std::vector<std::pair<int, int>>>>
      best_info;
  double best_time;
  double inner_time;
  int best_mem;
  ...
  for (DPNode *dp_node : id2node[end_level]->dp_nodes) {
    int cnt = 0;
    for (int i = 0; i < dp_node->state.size(); ++i) {
      std::tie(best_time, inner_time, best_mem, path) =
          process_state(dp_node, dp_node->state.size() - i - 1);
      if (best_time > 0) {
        ++cnt;
        best_info.push_back(
            std::make_tuple(best_time, inner_time, best_mem, path));
        if (cnt == topk) {
          break;
        }
      }
  ...
  int ret_size = std::min(topk, int(best_info.size()));
  int path_len = 0;
  if (ret_size > 0) {
    path_len = static_cast<int>(std::get<3>(best_info[0]).size());
  }
  write_binary<int>(start_level, output);
  write_binary<int>(end_level, output);
  write_binary<int>(ret_size, output);
  write_binary<int>(path_len, output);
  for (int i = 0; i < ret_size; ++i) {
    best_time = std::get<0>(best_info[i]);
    inner_time = std::get<1>(best_info[i]);
    best_mem = std::get<2>(best_info[i]);
    write_binary<double>(best_time, output);
    write_binary<double>(inner_time, output);
    write_binary<int>(best_mem, output);
    for (auto &item : std::get<3>(best_info[i])) {
      write_binary<int>(item.first, output);
      write_binary<int>(item.second, output);
    }
  }
}
...
int main(int argc, char **argv) {
  if (argc != 4) {
    std::cout << "Usage: ./solver in_path out_path is_verbose" << std::endl;
    return 0;
  }
  ...
  for (auto &item : intervals) {
    ...
    for (int end_level : end_levels) {
      post_process(start_level, end_level, topk, output);
    }
