...
class IRGraph(IRCell):
    ...
    def __init__(self, 
        ...
        if input_tensors is None:
            input_tensors = IRGraph.get_inputs(nodes)
        if output_tensors is None:
            output_tensors = IRGraph.get_outputs(nodes)
    ...
    def forward(self, *args) -> Union[IRTensor, Tuple[IRTensor]]:
        ...
        cell = IRCell(
            name = 'dataloader',
            signature = 'dataloder.__next__',
            input_length = 0,
            output_length = len(args)
        )
        for idx, arg in enumerate(args):
            cell.set_output(idx, arg)

        devices = set()
        for idx, arg in enumerate(args):
            cell.set_output(idx, arg)
            if isinstance(arg, IRTensor):
                for node in arg.dst(fgraph.nodes()):
                    devices.update(set(node.device))
        cell.device = list(devices)

        dataloader = IRGraph([cell], None, None, 'dataloader')
        data_sus = logic_translator(
            dataloader,
            su_type=SUType.Dataloader
        )
        ...
        # forward su
        fsus = logic_translator(fgraph, su_type=SUType.Forward)
        ...
        for input, output in zip(bgraph.inputs(), outputs):
            bgraph.replace_tensor(input, output)

        # backward su
        bsus = logic_translator(bgraph, su_type=SUType.Backward)
    ...
    @staticmethod
    def get_inputs(nodes: List[IRCell]) -> List[IRTensor]:
        """
    ...
    @staticmethod
    def get_outputs(nodes: List[IRCell]) -> List[IRTensor]:
        """
        Get all the input tensors the is not generated by nodes
