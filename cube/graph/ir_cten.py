...
class IRCell:
    ...
    def add_predecessor(self, input_index: int, node):
        """
        ...
        if node not in self._predecessors[input_index]:
            self._predecessors[input_index].append(node)

    def add_successor(self, output_index: int, node):
        """
        ...
        if not isinstance(node, IRCell):
            raise TypeError("Expected node to be IRCell")
        if node not in self._successors[output_index]:
