...
class RVDLayout:
    ...
    def incr(self, chunks: int, devices: Optional[np.ndarray] = None) -> Tuple:
        ...
        @return ret List[Tuple[RVDLayout, List[IRAdapterPrim]], ...]:
            tuple of pairs of <layout, [prims]> with each has a different device mapping.
        """
        rvd = list(self.vec)
        rvd[0] = rvd[0] * chunks
        olayout = RVDLayout.grid(self.ftensor, rvd[0], rvd[1], rvd[2:])
        # set device
        ...
        # setup prims
        imat = RVDLayout.dims2last(self.mat, [0]).flatten()
        omat = RVDLayout.dims2last(olayout.mat, [0]).reshape(-1, chunks)
        prims = []
    ...
    def decr(self, chunks: int, devices: Optional[np.ndarray] = None) -> Tuple:
        ...
        if devices is not None:
            ...
            for tensor, devid in zip(olayout.mat.flatten(), devices.flatten()):
                ...
                tensor.cell.device = int(devid)
        # setup prims
        imat = RVDLayout.dims2last(self.mat, [0]).reshape(-1, chunks)
        omat = RVDLayout.dims2last(olayout.mat, [0]).flatten()
        prims = []
    ...
    def incd(self, chunks: int, dim: int, devices: Optional[np.ndarray] = None) -> Tuple:
        ...
        if devices is not None:
            ...
            for tensor, devid in zip(olayout.mat.flatten(), devices.flatten()):
                ...
                tensor.cell.device = int(devid)
        # setup prims
        imat = RVDLayout.dims2last(self.mat, [2+dim]).flatten()
        omat = RVDLayout.dims2last(olayout.mat, [2+dim]).reshape(-1, chunks)
        prims = []
        for src, dsts in zip(imat, omat):
    ...
    def decd(self, chunks: int, dim: int, devices: Optional[np.ndarray] = None) -> Tuple:
        ...
        if devices is not None:
            ...
            for tensor, devid in zip(olayout.mat.flatten(), devices.flatten()):
                ...
                tensor.cell.device = int(devid)
        # setup prims
        imat = RVDLayout.dims2last(self.mat, [2+dim]).reshape(-1, chunks)
        omat = RVDLayout.dims2last(olayout.mat, [2+dim]).flatten()
        prims = []
        for srcs, dst in zip(imat, omat):
    ...
    def incv(self, chunks: int, devices: Optional[np.ndarray] = None) -> Tuple:
        ...
        if devices is not None:
            ...
            for tensor, devid in zip(olayout.mat.flatten(), devices.flatten()):
                ...
                tensor.cell.device = int(devid)
        # setup prims
        imat = RVDLayout.dims2last(self.mat, [1]).flatten()
        omat = RVDLayout.dims2last(olayout.mat, [1]).reshape(-1, chunks)
        prims = []
        for src, dsts in zip(imat, omat):
    ...
    def decv(self, chunks: int, devices: Optional[np.ndarray] = None) -> Tuple:
        ...
        if devices is not None:
            ...
            for tensor, devid in zip(olayout.mat.flatten(), devices.flatten()):
                ...
                tensor.cell.device = int(devid)
        # setup prims
        imat = RVDLayout.dims2last(self.mat, [1]).reshape(-1, chunks)
        omat = RVDLayout.dims2last(olayout.mat, [1]).flatten()
        prims = []
        for srcs, dst in zip(imat, omat):
