...
tp_ranks = list(range(args.tp_size))
_tp_group = -1
...
def stride_qk(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
    ...
    v = v.view(N * num_head, L // block_size, block_size, dim_head) # (N h) nblock blksize d
    ...
    # (N h) 1 blksize d
    pad_k_zero = torch.zeros_like(k[:,-3:-2])
    # (N h) 1 (3 blksize) d
    sliding_bottom_keys = torch.cat((pad_k_zero, k[:,-2:-1], k[:,-1:]), dim=2)
    ...
    # (N h) (nblock-3) (3 blksize) d
    stride_vals = torch.cat((v[:, 1:-2], v[:, 2:-1], k[:, 3:]), dim=2)
    # (N h) 1 (3 blksize) d
    stride_bottom_vals = torch.cat((pad_k_zero, v[:,-2:-1], v[:,-1:]), dim=2)
    # (N h) (nblock-2) (3 blksize) d
...
def randn_qk(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor,
    ...
    v = v.view(N * num_head, L // block_size, block_size, dim_head)
    # (N h) nblock-2 (2 blksize) d
    gathered_k = torch.cat(
        (k[:,indices[:,0]], k[:,indices[:,1]]), dim=2
    )
    # (N h) nblock-2 (2 blksize) d
    gathered_v = torch.cat(
        (v[:,indices[:,0]], v[:,indices[:,1]]), dim=2
    )
    # (N h) nblock blksize d
...
def sparse_attn(query: torch.Tensor,
    ...
    sqk, sqk_v = stride_qk(q, k, v, h, block_size=block_size)
