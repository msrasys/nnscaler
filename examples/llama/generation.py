...
from examples.llama.model import ModelArgs, Transformer
from examples.llama.tokenizer import Tokenizer
...
class Llama:
    ...
    def build(
        ...
        return Llama(model, tokenizer, use_cube)
    ...
    def __init__(self, model: Transformer, tokenizer: Tokenizer, use_cube: bool):
        ...
        if use_cube:
            print(f"Build using cube engine")
            CompileFlag.disable_code_line_info = False
    ...
    def generate(
        ...
        for cur_pos in range(min_prompt_len, total_len):
            if self.use_cube:
                logits = self.infer_fn[0](self.model, tokens[:, prev_pos:cur_pos], prev_pos)
            else:
