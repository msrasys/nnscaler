...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # TODO: support register function with kwargs on tensor
        # xq, xk = apply_rotary_emb(xq, xk, freqs_cis=freqs_cis)
        xq, xk = apply_rotary_emb(xq, xk, freqs_cis)

        # modification: move `.to(xq)` to the belowing
        ...
        keys = self.cache_k.to(xq)[:bsz, : start_pos + seqlen]
        values = self.cache_v.to(xq)[:bsz, : start_pos + seqlen]
...
class Transformer(nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        ...
        # TODO: support tracking dependency on kwarg IRObject
        ...
        #     )
        #     mask = torch.triu(mask, diagonal=start_pos + 1).type_as(h)
        mask = create_mask(tokens, h, start_pos)
