...
class ParallelSpec:
    ...
    def est_latency(self) -> float:
        return max(s.est_latency for s in self.stages)
    ...
    def save(self, filename: str):
        ...
        with open(filename, 'w') as f:
            json.dump([s.to_dict() for s in self.stages], f)

    def getstate(self) -> str:
        """
        Get plan state as json string
        """
        return json.dumps([s.to_dict() for s in self.stages])
    ...
    def loadstate(state: str):
        ...
        """
        stages = json.loads(state)
        return ParallelSpec(tuple(StageSpec.from_dict(s) for s in stages))
    ...
    def load(filename: str, check_graph_consistent: IRGraph = None):
        ...
        with open(filename, 'r') as f:
            stages = json.load(f)
        spec = ParallelSpec(tuple(StageSpec.from_dict(s) for s in stages))
    ...
    def __repr__(self) -> str:
        dscp = f'nstages: {len(self.stages)} | latency: {self.est_latency} ms'
