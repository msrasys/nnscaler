...
def transform_policy(graph: IRGraph, resource):
    ...
    sub_nodes = graph.partition(linear1, algo, config=dict(chunk_num=ndevs))
    all_sub_nodes.append(sub_nodes)

    algo = gelu.algorithms('dim')
    sub_nodes = graph.partition(gelu, algo, config=dict(dim=2, chunk_num=ndevs))
    all_sub_nodes.append(sub_nodes)

    algo = linear2.algorithms('row')
    sub_nodes = graph.partition(linear2, algo, config=dict(chunk_num=ndevs))
    all_sub_nodes.append(sub_nodes)
