...
class MegatronMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., tp_group=-1):
        ...
        self.fc1 = ColumnParallelLinear(in_features, hidden_features, full_input=True, full_output=False, tp_group=tp_group)
        self.act = act_layer()
        # self.fc2 = nn.Linear(hidden_features, out_features)
        self.fc2 = RowParallelLinear(hidden_features, out_features, full_input=False, full_output=True, tp_group=tp_group)
...
class MegatronWindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., tp_group=-1):
        ...
        self.qkv = ColumnParallelLinear(dim, dim * 3, bias=qkv_bias, full_input=True, full_output=False, tp_group=tp_group)
        self.attn_drop = nn.Dropout(attn_drop)
        # self.proj = nn.Linear(dim, dim)
        self.proj = RowParallelLinear(dim, dim, full_input=False, full_output=True, tp_group=tp_group)
