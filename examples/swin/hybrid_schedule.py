...
def send(tensors, to_rank, group):
    ...
    if group is not None:
        ...
        # print(f'send: {torch.distributed.get_rank()} -> {to_rank}: {tensors[0].shape}')
...
def scheduling_1f1b(model, inputs, bs, micro_bs, dtype, group):
    ...
    for i in range(num_warmup_microbatches):
        ...
        feature_map = recv(
            (torch.Size(model.in_size),), myrank-1, dtype, group
        )[0]
        image = inputs[i][0]
        # forward
    ...
    for i in range(num_warmup_microbatches):
        ...
        output_tensor_grad = recv(
            (torch.Size(model.out_size),), myrank+1, dtype, group
        )[0]
        # backward
        input_tensor_grad = backward_step(feature_map, output_tensor, output_tensor_grad)
