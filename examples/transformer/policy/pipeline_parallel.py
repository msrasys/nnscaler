...
def transform_policy(graph, resource):
    ...
    micro_batch_num = _batch_size // _micro_batch_size
    ...
    for node in graph.nodes():
        if isinstance(node, IRDataOperation) or isinstance(node, IRFwOperation):
            ...
            if algo is not None:
                ...
                sub_nodes = graph.partition(node, algo, config=dict(dim=1, chunk_num=micro_batch_num))
            for idx, sub_node in enumerate(sub_nodes):
                sub_node.tag = idx
    print('> [Done] transforming graph...')
    return graph
...
def schedule_policy(sugraph: SUGraph, resource):
    ...
    The schedule policy
    """
    print('> scheduling su graph...')
    num_micro_batch = _batch_size // _micro_batch_size
    ...
    print('> [Done] scheduling su graph')
    # print(sugraph)
    return sugraph
